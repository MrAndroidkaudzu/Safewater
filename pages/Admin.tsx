      ...
      <main className="flex-grow p-8 overflow-y-auto h-screen">
        ...
        <div className="md:hidden flex justify-between items-center mb-8">
          ...
          <div className="flex gap-2">
             ...
             <button onClick={handleLogout} className="p-2 bg-gray-200 rounded"><LogOut size={20}/></button>
          </div>
        </div>
                ...
                <tbody className="divide-y divide-gray-100">
                  {posts.map(post => (
                    <tr key={post.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-600">{new Date(post.date).toLocaleDateString()}</td>
